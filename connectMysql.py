...
def getPhoto(urls_of_photo, filename_path):
    ...
    with open(filename_path + '/urls_of_photo_' + info + '.txt', 'w', encoding='utf-8') as txt:
        for i in urls_of_photo:
            txt.write(i + '\n')


def getVideoFromYouTube(info, urls_of_video, filename_path):
...
db = pymysql.connect(host='localhost', user='root', password='1234567', db='download_info', charset='utf8')
cursor = db.cursor()
                ...
                for i in range(len(media)):
                    ...
                    if media_type == 'photo' or media_type == 'video':
                        if media_type == 'photo':
                            ...
                            path = files[2] + '\\' + status.id_str + '.mp4'
...
if __name__ == '__main__':
    thread = []
    t1 = threading.Thread(target=getPhoto, args=(image_files, files[1]))
    t2 = threading.Thread(target=downloadVideo, args=(video_files, index_of_video, files[2]))
    t3 = threading.Thread(target=getVideoFromYouTube, args=(info, urls_of_video, files[3]))
    t4 = threading.Thread(target=getAudio, args=(info, urls_of_audio, files[4]))
    thread.append(t1)
    thread.append(t2)
    thread.append(t3)
    thread.append(t4)
