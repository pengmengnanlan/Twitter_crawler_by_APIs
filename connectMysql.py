...
def getTweets(api, info):
    ...
    for tweet in tweepy.Cursor(api.user_timeline, screen_name=info, tweet_mode='extended').items(3):
...
def getVideo(urls_of_video, filename_path):
...
def getAudio(urls_of_audio, filename_path):
...
def getPhotoUrls(url):
    demo = getHTMLText(url)
    soup = BeautifulSoup(demo, 'html.parser')
    tag = ''
    for t in soup.find_all(name='script'):
        tag += str(t)
    tag = re.findall(r'live.staticflickr.com.+._o+\.jpg', tag)
    result = re.split('\\\\', (re.split(':"', tag[0])[-1]))
    photo_url = ''
    for s in result:
        if s:
            photo_url += s
    ...
    for status in all_tweets:
        ...
        try:
            for u in status.entities['urls']:
                if isPhoto(u['expanded_url']):
                    type_flag = 1
                    cursor.execute(tweet_sql,
                                   [status.user.id_str, status.user.screen_name, status.id_str, status.created_at,
                                    message, status.user.location])
                    db.commit()
                    url = getPhotoUrls(u['expanded_url'])
                    name = re.split('/', url)[-1]
                    path = files[1]+'\\'
                    urls_of_photo.add(url)
                    cursor.execute(media_sql,
                                   [time.strftime("%Y-%m-%d %H:%M:%S"), status.user.id_str, status.user.screen_name,
                                    status.id_str, type_flag, u['expanded_url'], name, path])
                    db.commit()
...
if __name__ == '__main__':
    thread = []
    t1 = threading.Thread(target=getPhoto, args=(urls_of_photo, files[0]))
    # t2 = threading.Thread(target=downloadVideo, args=(video_files, index_of_video, files[0]))
    t3 = threading.Thread(target=getVideo, args=(urls_of_video, files[0]))
    t4 = threading.Thread(target=getAudio, args=(urls_of_audio, files[0]))
    thread.append(t1)
    # thread.append(t2)
    thread.append(t3)
    thread.append(t4)
